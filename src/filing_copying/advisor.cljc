(ns filing-copying.advisor
  "FilingAdvisor — proposes a filing-and-copying operation (file a
  document, copy a document, handle a confidential or legally
  privileged document, destroy a document past retention policy) for
  a registered mandate. The advisor is swappable: `mock-advisor`
  ...
  of mandate provenance or confidential/retention risk;
  `filing-copying.governor` is the independent system that decides
  whether the proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :file|:copy|:handle-privileged-document|:destroy-past-retention-document
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for mandate " (:mandate-id request))})
...
(def ^:private system-prompt
  "You are a filing-and-copying advisor. Given a filing or copying
