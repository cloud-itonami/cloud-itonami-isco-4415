(ns filing-copying.actor
  "FilingCopyingActor — the ISCO-08 4415 independent filing-and-
  copying actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  filing/copying operation request (intake → advise → govern →
  ...
  The unconditional invariant: the FilingAdvisor can never directly
  commit a record or dispatch a robot action the FilingCopyingGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [filing-copying.advisor :as advisor]
            [filing-copying.governor :as governor]
            [filing-copying.store :as store]))

(defn build-graph
  "Build a compiled FilingCopyingActor graph. `store` implements
  `filing-copying.store/Store`. `advisor` implements
  `filing-copying.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:mandate-id (:mandate-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
