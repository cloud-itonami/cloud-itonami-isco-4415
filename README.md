...
- `src/filing_copying/store.cljk` — `Store` protocol +
  `MemStore`: registered client mandates, committed records, an
  append-only audit ledger.
- `src/filing_copying/advisor.cljk` — `Advisor` protocol;
...
- `src/filing_copying/governor.cljk` —
  ...
  sign-off.
- `src/filing_copying/actor.cljk` — `build-graph`, `run-request!`,
